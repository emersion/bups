...
format=     change output format (e.g. json)
"""
...
def do_root(n, sparse, owner_map, restore_root_meta = True):
    ...
    try:
        ...
        print_info(n, '.')
        total_restored += 1
        plog('Restoring: %d\r' % total_restored, { 'files_done': total_restored })
        for sub in n:
            m = None
